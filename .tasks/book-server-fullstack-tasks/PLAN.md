...
- [x] `task-06-importers-inpx-fb2.md`: Importers (Inpx+ZIP, fb2) — Suggested agent: Code — Covers: R4, S3, S4 — ✅ done (commit: `69a9548`); BookImporter/ImporterRegistry + InpxZipImporter/Fb2FolderImporter, async ImportWorker (PENDING→RUNNING→SUCCEEDED/FAILED), LocalBookFileStorage с md5-шардингом + path-traversal guard, md5-дедуп. Bugfix: flush очищенных composite-key связей перед повторным add (устранён `ObjectDeletedException` при повторном импорте); залогировано проглатываемое исключение импорта. 26 backend-тестов зелёные (`backend:test`).
- [x] `task-07-conversion-architecture.md`: Conversion architecture (без реализации) — Suggested agent: Code — Covers: R5, S6 — ✅ done (commit: `6d186f8`); FormatConverter/FormatPair/SupportedFormat + FormatConverterRegistry, `NotImplementedFormatConverter` (единственный prod-бин, всегда `Conversion not implemented yet`), ConversionService (fail-fast 422 через `ConverterNotFoundException`) + async ConversionWorker (PENDING→RUNNING→SUCCEEDED/FAILED), `POST/GET /api/conversions`, conversion-architecture.md. 28 backend-тестов зелёные (`backend:test`).
- [x] `task-08-book-lists-public-share-qr.md`: Book lists + public share + QR — Suggested agent: Code — Covers: R6, S7 — ✅ done (commit: `a2447a7`); private CRUD `/api/lists` + items add/remove/reorder с owner-guard (`ListAccessGuard`: 404 missing / 403 non-owner), `ShareService` (URL-safe 24-byte token, идемпотентный share + `regenerate=true`, revoke), `QrCodeService` (zxing → PNG), `ShareLinkDto{token, publicUrl, qrPngBase64}`, public `/api/public/lists/{token}` + `/qr.png` без auth (404 после revoke). 33 backend-теста зелёные (`backend:test`).
- [x] `task-09-frontend-vue-vuetify.md`: Frontend Vue 3 + Vuetify 3 — Suggested agent: Code — Covers: R7, R9, S2, S5, S6, S7, S8, S9 — ✅ done (commit: `489103b`); Vue 3.5 + Vuetify 3 + Vite + TS strict + Pinia + Vue Router SPA в `frontend/`. 13 views + 11 components + auth-store (localStorage JWT) + axios http с single-flight 401 refresh+retry + router-guard (`/p/lists/:token` public). API-клиенты под весь REST-контракт (books/authors/genres/lists/public/imports/conversions). `npm run build` (vue-tsc --noEmit + vite) зелёный, проверено независимо (`✓ built in 1.26s`). node_modules/dist в .gitignore.
- [x] `task-10-docker-compose-readme.md`: Docker-compose + README — Suggested agent: Code — Covers: R8, S1 — ✅ done (commit: `e5a784b`); `docker-compose.yml` (postgres 16-alpine + healthcheck → backend prod-профиль depends_on healthy → frontend nginx; volumes pgdata/books-data/imports-data), multi-stage `backend/Dockerfile` (temurin 25-jdk `bootJar -x test` → 25-jre non-root), `frontend/Dockerfile` (node20 → nginx:alpine), `nginx.conf` (SPA fallback + `/api` proxy), `.env.example`, полный README. `docker compose config` валиден, `backend:bootJar` собирается.
